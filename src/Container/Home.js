...
import {SET_SEARCH_TERM} from "../Context/Action/SearchAction"
...
function Home(){
    ...
    const searchTerm=useSelector((state)=>state?.searchTerm?.searchTerm ? state?.searchTerm :"")
// console.log(searchTerm)
    ...
    return(
        ...
        <div className={`w-2 ${isSideMenu ? "w-2" :"flex-[.2] xl:flex[.2"}
         min-h-screen max-h-screen relative bg-secondary px-3 py-6 flex flex-col items-center justify-start gap-4 transition-all
         duration-200 easy-in-out`}>
            ...
            <div className="overflow-hidden w-full flex flex-col gap-4">
                ...
                className="object-contain w-72 h-auto"></img>
                </Link>
                <Link to={"/newProject"}>
                    <div className="px-6 py-3 flex items-center justify-center rounded-xl border
         ...
         px-4 md:px-12 py-4 md:py-12">
            ...
            <div className="w-full flex items-center justify-center gap-3">
                <div className="bg-secondary w-full px-4 py-3 rounded-md flex items-center justify-center gap-3">
                     ...
                     placeholder="Search here for project"
...

                {
                    !user &&(
                        <motion.div whileTap={{scale:0.9}} className="flex items-center justify-center gap-3">
                            <Link to={"/home/auth"} 
                            className="bg-emerald-500 px-6 py-2 rounded-md text-white text-lg cursor-pointer
